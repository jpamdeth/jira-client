  ...
  def index
    begin
      ...
      if teams.empty? || statuses.empty?
        ...
        @issues = JiraTeamHelper.issue_search(teams, statuses, resolutions, created, updated, resolved, unassigned, unpointed)
        if @issues.size >= 1000
  ...
  def velocity
    begin
      ...
      if @team.nil?
        ...
        all_issues = JiraTeamHelper.velocity(@team)
        @buckets = JiraTeamHelper.bucket_tickets(all_issues)
      end
    rescue => e
      @error = e.message
    end
